...
export const BrandSettingsAdmin = () => {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const toSave = {
      ...settings,
      overlayOpacity: settings.overlayOpacity ? Number(settings.overlayOpacity) : 0,
      blur: settings.blur ? Number(settings.blur) : 0,
      brightness: settings.brightness ? Number(settings.brightness) : 100
    };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="bg-white rounded-3xl shadow-sm border border-stone-200 overflow-hidden">
        <div className="p-6 md:p-8 border-b border-stone-100 bg-stone-50 flex items-center justify-between">
          <div>
        ...
        <form onSubmit={handleSave} className="p-6 md:p-8 space-y-10">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-semibold text-stone-700 mb-1.5">Site Logo URL</label>
          ...
          <section>
            <div className="flex items-center gap-2 mb-6 pb-2 border-b border-stone-100">
              ...
              <h3 className="text-lg font-bold text-stone-800">Site Background</h3>
            </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="relative">
                  <Upload className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-stone-400" />
                  <input
                    type="text"
                    value={settings.backgroundImageUrl}
                    onChange={(e) => setSettings({ ...settings, backgroundImageUrl: e.target.value })}
                    placeholder="/background.jpg or https://..."
                    className="w-full pl-9 pr-4 py-2.5 bg-stone-50 border border-stone-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-amber-500 text-sm transition-all"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 bg-stone-50 border border-stone-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-amber-500 text-sm"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-2">
                ...
                <div>
                  <label className="block text-xs font-semibold text-stone-500 mb-1.5 uppercase tracking-wider">
                    Opacity ({settings.overlayOpacity !== undefined ? settings.overlayOpacity : 65}%)
                  </label>
                  <input
                    type="range"
                    min="0"
                ...
                <div>
                  <label className="block text-xs font-semibold text-stone-500 mb-1.5 uppercase tracking-wider">
                    Brightness ({settings.brightness !== undefined ? settings.brightness : 100}%)
                  </label>
                  <input
                    type="range"
                    min="50"
