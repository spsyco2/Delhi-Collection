...
export const AdminLayout: React.FC = () => {
  ...
  const [brandSettings, setBrandSettings] = useState(() => campaignStore.getBrandSettings());
  ...
  useEffect(() => {
    const unsubscribe = campaignStore.subscribe(() => {
      ...
      setBrandSettings(campaignStore.getBrandSettings());
    });
    return unsubscribe;
  }, []);
    ...
    <div className="min-h-screen bg-stone-50 flex flex-col md:flex-row selection:bg-amber-900 selection:text-amber-50">
      ...
      <div className="md:hidden bg-white border-b border-stone-200 flex items-center justify-between p-4 sticky top-0 z-50 shadow-sm">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full overflow-hidden bg-stone-900 border border-amber-400/40 flex items-center justify-center shadow-md">
            {brandSettings.logoUrl ? (
              <img src={brandSettings.logoUrl} alt="Logo" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
            ) : (
              <span className="font-serif font-black text-amber-400 text-xs">DC</span>
            )}
          </div>
          <span className="font-serif font-bold text-lg text-stone-900">{brandSettings.brandName || 'Delhi Collection'}</span>
        </div>
      ...
      <aside className={`${mobileMenuOpen ? 'block' : 'hidden'} md:block w-full md:w-72 bg-white border-r border-stone-200 flex-shrink-0 flex flex-col fixed md:sticky top-[73px] md:top-0 h-[calc(100vh-73px)] md:h-screen z-40 shadow-xl md:shadow-none transition-all`}>
        ...
        <div className="hidden md:flex p-6 border-b border-stone-100 items-center gap-3">
          <div className="w-10 h-10 rounded-full overflow-hidden bg-stone-900 border border-amber-400/40 flex items-center justify-center shadow-md flex-shrink-0">
            {brandSettings.logoUrl ? (
              <img src={brandSettings.logoUrl} alt="Logo" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
            ) : (
              <span className="font-serif font-black text-amber-400 text-sm">DC</span>
            )}
          </div>
          <div className="min-w-0 flex-1">
            <h1 className="font-serif font-bold text-lg text-stone-900 leading-tight truncate">{brandSettings.brandName || 'Delhi Collection'}</h1>
        ...
        <div className="p-4 border-t border-stone-100">
           <div className="bg-stone-900 text-stone-300 rounded-xl p-3.5 border border-stone-800 flex items-center justify-between">
             <div className="flex items-center gap-2.5">
               <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
               <div>
                 <p className="text-xs font-semibold text-white">Store Admin</p>
                 <p className="text-[10px] text-stone-400">All controls active</p>
               </div>
             </div>
             <span className="text-[10px] px-2 py-0.5 rounded-md bg-stone-800 text-amber-400 font-mono font-semibold border border-stone-700">Live</span>
           </div>
        </div>
      </aside>
